...
def fragmentLengthDistribution(rfLenList):
    ...
    rfCommonLenList = rfCounter.most_common(len(rfCounter)) # [(length, count)...], ordered by count
    sortedirList = sorted(rfCommonLenList, key=lambda x:x[0]) # [(length, count)...], ordered by length
